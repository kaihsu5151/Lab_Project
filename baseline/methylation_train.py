# -*- coding: utf-8 -*-
"""
Methylation -> IC50 (Y) XGBoost CV pipeline (drug-wise)

This is adapted from your CNV pipeline (cnv_train.py):
- Same ID standardization (removes all non-alphanumeric)
- Same per-drug CV and outputs (cv_metrics_folds.csv, cv_predictions.csv, folds.npz, models/)
- Supports folds_mode="reuse" so you can share EXACT folds across omics (no leakage when ensembling)

Expected inputs in --data-dir:
- Methylation_final.csv         : wide table (rows=cell lines, cols=genes). First col is ID.
...
- good_drugs_summary.csv        : list of drugs to run (column 'drug')

Run:
python methylation_train.py --data-dir . --meth-file Methylation_final.csv --mapping-file cell_line_mapping.csv --drug-file processed_drug_response.csv --good-drugs-file good_drugs_summary.csv --top-n 2000 --use-gpu --calibrate

To reuse folds from another omics (recommended):
python methylation_train.py ... --folds-mode reuse --folds-root /path/to/results_cnv/<run_id>
Then per drug it will load: <folds_root>/<drug>/folds.npz

Note:
- We do train-only mean imputation before corr-FS and scaling.
"""
...
def load_meth_wide(meth_csv: str, target_keys: set[str], id_col: Optional[str]) -> pd.DataFrame:
    """
    Load methylation wide table and keep only target cell lines.
    Output index: 'std', columns: genes
    """
    head = pd.read_csv(meth_csv, nrows=5)
    # commonly the first col is "Unnamed: 0"
    id_col = id_col or pick_first_existing(head, ["Unnamed: 0", "CellLineName", "CCLENameStripped", "StrippedName", "Original_Name"], True)

    df = pd.read_csv(meth_csv)
    df = df.copy()
    df["std"] = df[id_col].apply(standardize_name)
    df = df.dropna(subset=["std"])
    df = df[df["std"].isin(target_keys)]
    if len(df) == 0:
        raise ValueError("No methylation rows matched target cell lines. Check mapping & naming.")

    gene_cols = [c for c in df.columns if c not in [id_col, "std"]]
    df[gene_cols] = df[gene_cols].apply(pd.to_numeric, errors="coerce")

    # duplicate std -> mean
    df = df.copy()
    df = df.groupby("std", as_index=False)[gene_cols].mean(numeric_only=True)
...
def run_cv_for_drug(
    ...
    *,
    ...
    sub_agg = sub_agg[sub_agg["std"].isin(meth_df.index)].copy()
    if len(sub_agg) < min_cell_lines:
        print(f"[SKIP] {drug_name}: only {len(sub_agg)} cell lines (<{min_cell_lines}) after matching methylation.")
        return None

    X = meth_df.loc[sub_agg["std"]].to_numpy(dtype=np.float32)
    ...
    for i, ((tr, te), idx) in enumerate(zip(folds, fold_feature_idx), 1):
        ...
        preproc = dict(
            ...
            drug=drug_name,
            fold=int(i),
            meth_gene_names=[str(g) for g in gene_names.tolist()],
    ...
    title = f"{drug_name} (Methylation OOF across folds)"
    subtitle = (
    ...
    save_scatter(
        ...
        y_true_all,
        y_pred_raw_all,
        title=f"{drug_name} (Methylation raw OOF across folds)",
...
def main():
    ...
    meth_df = load_meth_wide(meth_path, target_keys, id_col=args.meth_id_col)
