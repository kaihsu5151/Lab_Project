...
def run_cv_for_drug(
    ...
    *,
    ...
    # ----- feature selection per fold (train-only) -----
    fold_feature_idx = []
    for i, (tr, te) in enumerate(folds, 1):
        idx, sc = topn_by_corr(X[tr], y[tr], top_n, method=corr_method, use_abs=use_abs_corr)
        fold_feature_idx.append(idx)
        if top_n > 0:
            print(f"[{drug_name}] Fold {i}: selected {len(idx)} CNV genes, top1={gene_names[idx[0]]}, score={sc[0]:.4f}")
        else:
            print(f"[{drug_name}] Fold {i}: NO feature selection, using all {len(idx)} CNV genes")
...
def main():
    ...
    RESULTS_ROOT = os.path.join(
        DATA_DIR,
        "results_cnv",
        f"CNV__top{args.top_n}__{args.corr_method}__abs{int(args.use_abs_corr)}__cal{int(calibrate)}__gpu{int(args.use_gpu)}__{run_id}",
    )
