...
def theme_univariate(df: pd.DataFrame, theme_cols: list[str], group_col: str = 'delta_group', group_a: str = 'improve', group_b: str = 'worsen', theme_prefix: str = 'theme_') -> pd.DataFrame:
    rows = []
    sub = df[df[group_col].isin([group_a, group_b])].copy()
    group_a_set = set(sub.loc[sub[group_col] == group_a, 'drug'])
    group_b_set = set(sub.loc[sub[group_col] == group_b, 'drug'])
    ...
    for c in theme_cols:
        ...
        rows.append({
            'feature': c,
            'theme': c.replace(theme_prefix, '', 1) if str(c).startswith(theme_prefix) else str(c),
...
def theme_enrichment(delta_df: pd.DataFrame, theme_mat: pd.DataFrame, positive_group: str, opposite_group: Optional[str] = None, theme_prefix: str = 'theme_') -> pd.DataFrame:
    if theme_mat.empty:
        return pd.DataFrame(columns=["source", "field", "token"])
    if opposite_group is None:
        pos_set = set(delta_df.loc[delta_df["delta_group"] == positive_group, "drug"])
        ...
        all_set = pos_set | set(delta_df.loc[delta_df["delta_group"] == opposite_group, "drug"])
    rows = []
    theme_cols = [c for c in theme_mat.columns if c.startswith(theme_prefix)]
    for c in theme_cols:
        token = c.replace(theme_prefix, "", 1) if str(c).startswith(theme_prefix) else str(c)
...
def lodo_theme_stability(df: pd.DataFrame, theme_cols: list[str], theme_prefix: str = 'theme_') -> pd.DataFrame:
    sub = df[df['delta_group'].isin(['improve', 'worsen'])].copy()
    drugs = sub['drug'].dropna().astype(str).tolist()
    rows = []
    for c in theme_cols:
...
def threshold_robustness_theme(df: pd.DataFrame, theme_cols: list[str], thresholds: list[float], theme_prefix: str = 'theme_') -> tuple[pd.DataFrame, pd.DataFrame]:
    detail_rows = []
    for thr in thresholds:
        tmp = df.copy()
        tmp['delta_group_thr'] = build_delta_groups_from_threshold(tmp, thr)
        res = theme_univariate(tmp, theme_cols, group_col='delta_group_thr', group_a='improve', group_b='worsen', theme_prefix=theme_prefix)
        if res.empty:
            continue
...
def build_theme_token_filtered_sets(
    raw_token_uni: pd.DataFrame,
    raw_token_theme_catalog: pd.DataFrame,
    raw_token_lodo: Optional[pd.DataFrame] = None,
    raw_token_thr_summary: Optional[pd.DataFrame] = None,
    min_support: int = 2,
    min_abs_log2_or: float = 0.25,
    min_lodo_consistency: float = 0.8,
    min_threshold_consistency: float = 0.8,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    if raw_token_uni.empty or raw_token_theme_catalog.empty:
    ...
    detail = mapped.merge(raw_token_uni[cols].copy(), on=["feature", "raw_token"], how="left")

    if raw_token_lodo is not None and not raw_token_lodo.empty:
        keep_cols = [c for c in ["feature", "raw_token", "lodo_sign_consistency", "lodo_p_lt_0_05_rate", "lodo_majority_direction"] if c in raw_token_lodo.columns]
        detail = detail.merge(raw_token_lodo[keep_cols].copy(), on=["feature", "raw_token"], how="left")
    if raw_token_thr_summary is not None and not raw_token_thr_summary.empty:
        keep_cols = [c for c in ["feature", "raw_token", "threshold_sign_consistency", "threshold_p_lt_0_05_rate", "threshold_majority_direction"] if c in raw_token_thr_summary.columns]
        detail = detail.merge(raw_token_thr_summary[keep_cols].copy(), on=["feature", "raw_token"], how="left")

    detail["token_effect_sign"] = clean_numeric_series(detail.get("log2_odds_ratio", np.nan)).apply(effect_sign)
    detail["abs_log2_odds_ratio"] = clean_numeric_series(detail.get("log2_odds_ratio", np.nan)).abs()

    maj_rows = []
    for theme, g in detail.groupby("assigned_theme", sort=True):
        eff = clean_numeric_series(g.get("log2_odds_ratio", np.nan)).to_numpy(dtype=float)
        eff = eff[np.isfinite(eff)]
        maj = effect_sign(np.nanmedian(eff)) if eff.size else 0
        maj_rows.append({
            "assigned_theme": theme,
            "theme_majority_token_direction": "enriched_in_improve" if maj > 0 else ("enriched_in_worsen" if maj < 0 else "neutral_or_missing"),
            "theme_majority_token_sign": maj,
        })
    maj_df = pd.DataFrame(maj_rows)
    detail = detail.merge(maj_df, on="assigned_theme", how="left")

    detail["passes_support"] = clean_numeric_series(detail.get("n_drugs_supporting_token", np.nan)).fillna(0) >= float(min_support)
    detail["passes_effect_size"] = clean_numeric_series(detail.get("abs_log2_odds_ratio", np.nan)).fillna(0) >= float(min_abs_log2_or)
    detail["passes_lodo"] = clean_numeric_series(detail.get("lodo_sign_consistency", np.nan)).fillna(0) >= float(min_lodo_consistency)
    detail["passes_threshold"] = clean_numeric_series(detail.get("threshold_sign_consistency", np.nan)).fillna(0) >= float(min_threshold_consistency)

    token_sign = clean_numeric_series(detail.get("token_effect_sign", np.nan)).fillna(0).astype(int)
    theme_sign = clean_numeric_series(detail.get("theme_majority_token_sign", np.nan)).fillna(0).astype(int)
    detail["aligns_with_theme_direction"] = (token_sign == theme_sign) & (theme_sign != 0)

    detail["token_keep_flag"] = (
        detail["passes_support"] &
        detail["passes_effect_size"] &
        detail["passes_lodo"] &
        detail["passes_threshold"] &
        detail["aligns_with_theme_direction"]
    )

    def _wmean(vals, weights):
        vals = clean_numeric_series(vals).to_numpy(dtype=float)
        weights = clean_numeric_series(weights).to_numpy(dtype=float)
    ...
    sum_rows = []
    for theme, g in detail.groupby("assigned_theme", sort=True):
        all_eff = clean_numeric_series(g.get("log2_odds_ratio", np.nan)).to_numpy(dtype=float)
        all_eff = all_eff[np.isfinite(all_eff)]
        kept = g[g["token_keep_flag"]].copy()
        kept_eff = clean_numeric_series(kept.get("log2_odds_ratio", np.nan)).to_numpy(dtype=float)
        kept_eff = kept_eff[np.isfinite(kept_eff)]

        all_mean = float(np.nanmean(all_eff)) if all_eff.size else np.nan
        kept_mean = float(np.nanmean(kept_eff)) if kept_eff.size else np.nan
        all_med = float(np.nanmedian(all_eff)) if all_eff.size else np.nan
        kept_med = float(np.nanmedian(kept_eff)) if kept_eff.size else np.nan
        all_w = _wmean(g.get("log2_odds_ratio", np.nan), g.get("n_drugs_supporting_token", np.nan))
        kept_w = _wmean(kept.get("log2_odds_ratio", np.nan), kept.get("n_drugs_supporting_token", np.nan))

        kept_tokens = kept.sort_values(["abs_log2_odds_ratio", "n_drugs_supporting_token"], ascending=[False, False])["raw_token"].dropna().astype(str).tolist()
        dropped_tokens = g.loc[~g["token_keep_flag"], "raw_token"].dropna().astype(str).tolist()

        sum_rows.append({
            "theme": theme,
            "theme_majority_token_direction": g["theme_majority_token_direction"].iloc[0] if len(g) else np.nan,
            "n_tokens_total": int(len(g)),
            "n_tokens_with_effect": int(clean_numeric_series(g.get("log2_odds_ratio", np.nan)).notna().sum()),
            "n_tokens_kept": int(len(kept)),
            "prop_tokens_kept": safe_float(len(kept) / len(g)) if len(g) else np.nan,
            "mean_effect_all_tokens": all_mean,
            "median_effect_all_tokens": all_med,
            "weighted_mean_effect_all_tokens": all_w,
            "mean_effect_kept_tokens": kept_mean,
            "median_effect_kept_tokens": kept_med,
            "weighted_mean_effect_kept_tokens": kept_w,
            "refinement_gain_mean": kept_mean - all_mean if np.isfinite(kept_mean) and np.isfinite(all_mean) else np.nan,
            "refinement_gain_weighted_mean": kept_w - all_w if np.isfinite(kept_w) and np.isfinite(all_w) else np.nan,
            "kept_tokens": " | ".join(kept_tokens),
            "dropped_tokens": " | ".join(dropped_tokens),
        })
    summary = pd.DataFrame(sum_rows)
    if not summary.empty:
        summary = summary.sort_values(["weighted_mean_effect_kept_tokens", "n_tokens_kept", "prop_tokens_kept"], ascending=[False, False, False])
    detail = detail.rename(columns={"assigned_theme": "theme"})
    return detail, summary


def plot_refined_theme_gain(summary_df: pd.DataFrame, out_png: Path, topk: int = 12) -> None:
    if summary_df.empty or "refinement_gain_weighted_mean" not in summary_df.columns:
        return
    g = summary_df.copy()
    g = g[np.isfinite(clean_numeric_series(g["refinement_gain_weighted_mean"]))]
    if g.empty:
        return
    g = g.sort_values("refinement_gain_weighted_mean", ascending=False).head(topk).iloc[::-1]
    vals = clean_numeric_series(g["refinement_gain_weighted_mean"]).to_numpy(dtype=float)
    fig, ax = plt.subplots(figsize=(10, max(4, 0.4 * len(g))))
    ax.barh(g["theme"], vals)
    ax.set_xlabel("refined weighted mean − all-token weighted mean")
    ax.set_title("Themes with the largest gain after token filtering")
...
def build_refined_theme_presence_matrix(
    theme_token_filtered_detail: pd.DataFrame,
    raw_token_mat: pd.DataFrame,
    all_drugs: Optional[pd.Series] = None,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Build a refined theme presence matrix using only kept raw tokens within each theme.

    Important
    ---------
    Support counts in the catalog are computed *after* restricting to ``all_drugs``
    when that argument is provided. This keeps theme support aligned with the actual
    analysis universe and prevents impossible counts (for example support > total
    number of analyzed drugs).
    """
    if theme_token_filtered_detail.empty or raw_token_mat.empty:
        cols = ["drug"]
        if all_drugs is not None:
            return pd.DataFrame({"drug": pd.Series(all_drugs).dropna().astype(str).unique()}), pd.DataFrame()
        return pd.DataFrame(columns=cols), pd.DataFrame()

    kept = theme_token_filtered_detail[theme_token_filtered_detail['token_keep_flag']].copy()
    if kept.empty:
        if all_drugs is not None:
            return pd.DataFrame({"drug": pd.Series(all_drugs).dropna().astype(str).unique()}), pd.DataFrame()
        return pd.DataFrame(columns=["drug"]), pd.DataFrame()

    feature_to_theme = kept[['feature', 'theme', 'raw_token']].drop_duplicates().copy()
    raw_cols_present = [c for c in raw_token_mat.columns if c in set(feature_to_theme['feature'])]
    if not raw_cols_present:
        if all_drugs is not None:
            return pd.DataFrame({"drug": pd.Series(all_drugs).dropna().astype(str).unique()}), pd.DataFrame()
        return pd.DataFrame(columns=["drug"]), pd.DataFrame()

    base = raw_token_mat[['drug'] + raw_cols_present].copy()
    base['drug'] = base['drug'].astype(str)
    if all_drugs is not None:
        allowed = pd.Series(all_drugs).dropna().astype(str).unique().tolist()
        base = base[base['drug'].isin(set(allowed))].copy()
    for c in raw_cols_present:
        base[c] = clean_numeric_series(base[c]).fillna(0).astype(int)

    refined = pd.DataFrame({'drug': base['drug'].astype(str)})
    for theme, g in feature_to_theme.groupby('theme', sort=True):
        feats = [f for f in g['feature'].tolist() if f in base.columns]
        if not feats:
            continue
        col = f'refined_theme_{theme}'
        refined[col] = (base[feats].sum(axis=1) > 0).astype(int)

    if all_drugs is not None:
        drug_df = pd.DataFrame({'drug': pd.Series(all_drugs).dropna().astype(str).unique()})
        refined = drug_df.merge(refined, on='drug', how='left')
    val_cols = [c for c in refined.columns if c != 'drug']
    for c in val_cols:
        refined[c] = clean_numeric_series(refined[c]).fillna(0).astype(int)

    cat_rows = []
    for theme, g in feature_to_theme.groupby('theme', sort=True):
        feats = [f for f in g['feature'].tolist() if f in raw_cols_present]
        col = f'refined_theme_{theme}'
        supporting_drugs = int(refined[col].sum()) if col in refined.columns else 0
        cat_rows.append({
            'theme': theme,
            'refined_feature': col,
            'n_kept_tokens': int(len(g)),
            'kept_features': ' | '.join(feats),
            'kept_tokens': ' | '.join(sorted(g['raw_token'].dropna().astype(str).unique().tolist())),
            'n_drugs_supporting_refined_theme': supporting_drugs,
        })

    refined_catalog = pd.DataFrame(cat_rows)
    return refined, refined_catalog


def compare_original_vs_refined_theme_pvalues(original_theme_uni: pd.DataFrame, refined_theme_uni: pd.DataFrame) -> pd.DataFrame:
    if original_theme_uni.empty and refined_theme_uni.empty:
        return pd.DataFrame()
    orig = original_theme_uni.copy() if original_theme_uni is not None else pd.DataFrame()
    ref = refined_theme_uni.copy() if refined_theme_uni is not None else pd.DataFrame()
    if not orig.empty:
        orig = orig[['theme', 'feature', 'pvalue', 'fdr_bh', 'minus_log10_p', 'log2_odds_ratio', 'n_drugs_supporting_theme', 'support_rate_total', 'effect_direction']].copy()
        orig = orig.rename(columns={
            'feature': 'original_feature',
            'pvalue': 'original_pvalue',
            'fdr_bh': 'original_fdr_bh',
            'minus_log10_p': 'original_minus_log10_p',
            'log2_odds_ratio': 'original_log2_odds_ratio',
            'n_drugs_supporting_theme': 'original_n_drugs_supporting_theme',
            'support_rate_total': 'original_support_rate_total',
            'effect_direction': 'original_effect_direction',
        })
    if not ref.empty:
        ref = ref[['theme', 'feature', 'pvalue', 'fdr_bh', 'minus_log10_p', 'log2_odds_ratio', 'n_drugs_supporting_theme', 'support_rate_total', 'effect_direction']].copy()
        ref = ref.rename(columns={
            'feature': 'refined_feature',
            'pvalue': 'refined_pvalue',
            'fdr_bh': 'refined_fdr_bh',
            'minus_log10_p': 'refined_minus_log10_p',
            'log2_odds_ratio': 'refined_log2_odds_ratio',
            'n_drugs_supporting_theme': 'refined_n_drugs_supporting_theme',
            'support_rate_total': 'refined_support_rate_total',
            'effect_direction': 'refined_effect_direction',
        })
    out = orig.merge(ref, on='theme', how='outer') if not orig.empty else ref.copy()
    if orig.empty:
        out = ref.copy()
    elif ref.empty:
        out = orig.copy()
    out['pvalue_improvement'] = clean_numeric_series(out.get('original_pvalue', np.nan)) - clean_numeric_series(out.get('refined_pvalue', np.nan))
    out['minus_log10_p_gain'] = clean_numeric_series(out.get('refined_minus_log10_p', np.nan)) - clean_numeric_series(out.get('original_minus_log10_p', np.nan))
    out['fdr_improvement'] = clean_numeric_series(out.get('original_fdr_bh', np.nan)) - clean_numeric_series(out.get('refined_fdr_bh', np.nan))
    out['became_more_significant'] = clean_numeric_series(out.get('minus_log10_p_gain', np.nan)) > 0
    out = out.sort_values(['minus_log10_p_gain', 'refined_minus_log10_p', 'original_minus_log10_p'], ascending=[False, False, False])
    return out


def plot_refined_theme_pvalue_gain(compare_df: pd.DataFrame, out_png: Path, topk: int = 12) -> None:
    if compare_df.empty or 'minus_log10_p_gain' not in compare_df.columns:
        return
    g = compare_df.copy()
    g = g[np.isfinite(clean_numeric_series(g['minus_log10_p_gain']))]
    if g.empty:
        return
    g = g.sort_values('minus_log10_p_gain', ascending=False).head(topk).iloc[::-1]
    vals = clean_numeric_series(g['minus_log10_p_gain']).to_numpy(dtype=float)
    fig, ax = plt.subplots(figsize=(10, max(4, 0.4 * len(g))))
    ax.barh(g['theme'], vals)
    ax.set_xlabel(r'$-\log_{10}(p)$ gain after refined-theme filtering')
    ax.set_title('Themes with improved p-values after token filtering')
    annotate_barh(ax, vals)
    fig.tight_layout()
    fig.savefig(out_png, dpi=180)
    plt.close(fig)
...
def build_size_bias_scatter(*args, **kwargs):
    return


def build_size_bias_table(
    refined_theme_catalog: pd.DataFrame,
    refined_theme_uni: pd.DataFrame,
    refined_enrich_improve: pd.DataFrame,
    refined_enrich_worsen: pd.DataFrame,
    theme_refined_token_set: Optional[pd.DataFrame] = None,
) -> pd.DataFrame:
    if refined_theme_catalog is None or refined_theme_catalog.empty:
        return pd.DataFrame()
    out = refined_theme_catalog.copy()
    if theme_refined_token_set is not None and not theme_refined_token_set.empty:
        tmp = theme_refined_token_set[[c for c in [
            'theme', 'weighted_mean_effect_all_tokens', 'weighted_mean_effect_kept_tokens',
            'mean_effect_all_tokens', 'mean_effect_kept_tokens',
            'refinement_gain_weighted_mean', 'refinement_gain_mean',
            'n_tokens_total', 'n_tokens_kept', 'prop_tokens_kept'
        ] if c in theme_refined_token_set.columns]].copy()
        out = out.merge(tmp, on='theme', how='left')
    if refined_theme_uni is not None and not refined_theme_uni.empty:
        tmp = refined_theme_uni[[c for c in ['theme', 'pvalue', 'minus_log10_p', 'fdr_bh', 'log2_odds_ratio', 'n_drugs_supporting_theme'] if c in refined_theme_uni.columns]].copy()
        tmp = tmp.rename(columns={
            'pvalue': 'refined_two_sided_pvalue',
            'minus_log10_p': 'refined_two_sided_minus_log10_p',
            'fdr_bh': 'refined_two_sided_fdr_bh',
            'log2_odds_ratio': 'refined_two_sided_log2_odds_ratio',
            'n_drugs_supporting_theme': 'refined_two_sided_support_count',
        })
        out = out.merge(tmp, on='theme', how='left')
    if refined_enrich_improve is not None and not refined_enrich_improve.empty:
        tmp = refined_enrich_improve[[c for c in ['token', 'pvalue', 'minus_log10_p', 'fdr_bh', 'odds_ratio', 'n_improve', 'n_with_token', 'n_total'] if c in refined_enrich_improve.columns]].copy()
        tmp = tmp.rename(columns={
            'token': 'theme',
            'pvalue': 'refined_improve_enrichment_pvalue',
            'minus_log10_p': 'refined_improve_enrichment_minus_log10_p',
            'fdr_bh': 'refined_improve_enrichment_fdr_bh',
            'odds_ratio': 'refined_improve_enrichment_odds_ratio',
            'n_improve': 'refined_improve_n_positive',
            'n_with_token': 'refined_improve_n_with_theme',
            'n_total': 'refined_improve_n_total',
        })
        tmp['refined_improve_enrichment_log2_odds_ratio'] = clean_numeric_series(tmp['refined_improve_enrichment_odds_ratio']).map(lambda x: np.log2(x) if np.isfinite(x) and x > 0 else np.nan)
        out = out.merge(tmp, on='theme', how='left')
    if refined_enrich_worsen is not None and not refined_enrich_worsen.empty:
        tmp = refined_enrich_worsen[[c for c in ['token', 'pvalue', 'minus_log10_p', 'fdr_bh', 'odds_ratio', 'n_worsen', 'n_with_token', 'n_total'] if c in refined_enrich_worsen.columns]].copy()
        tmp = tmp.rename(columns={
            'token': 'theme',
            'pvalue': 'refined_worsen_enrichment_pvalue',
            'minus_log10_p': 'refined_worsen_enrichment_minus_log10_p',
            'fdr_bh': 'refined_worsen_enrichment_fdr_bh',
            'odds_ratio': 'refined_worsen_enrichment_odds_ratio',
            'n_worsen': 'refined_worsen_n_positive',
            'n_with_token': 'refined_worsen_n_with_theme',
            'n_total': 'refined_worsen_n_total',
        })
        out = out.merge(tmp, on='theme', how='left')
    out = out.sort_values(['refined_two_sided_minus_log10_p', 'n_kept_tokens', 'n_drugs_supporting_refined_theme'], ascending=[False, False, False])
    return out




def plot_size_bias_ranked_panel(
    df: pd.DataFrame,
    out_png: Path,
    topk: int = 12,
    left_metric: str = 'refined_improve_enrichment_log2_odds_ratio',
    left_title: str = 'Improve vs rest effect size',
    left_xlabel: str = 'theme-level log2 odds ratio',
) -> None:
    """Three-panel ranked summary with effect size on the left.

    Left panel shows a refined theme effect-size metric instead of an enrichment
    p-value. Middle and right panels preserve theme size and refined support.
    """
    needed = {'theme', left_metric, 'n_kept_tokens', 'n_drugs_supporting_refined_theme'}
    if df.empty or not needed.issubset(df.columns):
        return
    g = df.copy()
    g[left_metric] = clean_numeric_series(g[left_metric])
    g['n_kept_tokens'] = clean_numeric_series(g['n_kept_tokens'])
    g['n_drugs_supporting_refined_theme'] = clean_numeric_series(g['n_drugs_supporting_refined_theme'])
    g = g[np.isfinite(g[left_metric])].copy()
    if g.empty:
        return
    g = g.sort_values([left_metric, 'n_kept_tokens', 'n_drugs_supporting_refined_theme'], ascending=[False, False, False]).head(topk)
    g = g.iloc[::-1].reset_index(drop=True)
    y = np.arange(len(g))
    fig, axes = plt.subplots(1, 3, figsize=(14, max(4.5, 0.42 * len(g))), gridspec_kw={'width_ratios': [3.2, 1.4, 1.8]})
    ax0, ax1, ax2 = axes

    vals0 = g[left_metric].to_numpy(dtype=float)
    ax0.barh(y, vals0)
    ax0.set_yticks(y)
    ax0.set_yticklabels(g['theme'])
    ax0.set_xlabel(left_xlabel)
    ax0.set_title(left_title)
    annotate_barh(ax0, vals0)

    vals1 = g['n_kept_tokens'].to_numpy(dtype=float)
    ax1.barh(y, vals1)
    ax1.set_yticks(y)
    ax1.set_yticklabels([])
    ax1.set_xlabel('kept\ntokens')
    ax1.set_title('Theme size')
    annotate_barh(ax1, vals1)

    vals2 = g['n_drugs_supporting_refined_theme'].to_numpy(dtype=float)
    ax2.barh(y, vals2)
    ax2.set_yticks(y)
    ax2.set_yticklabels([])
    ax2.set_xlabel('supporting\ndrugs')
    ax2.set_title('Refined support')
    annotate_barh(ax2, vals2)

    fig.suptitle('Do top refined themes stay top (improve vs rest)?')
    fig.tight_layout()
    fig.savefig(out_png, dpi=180)
    plt.close(fig)


def plot_size_matched_bin_summary(df: pd.DataFrame, out_png: Path) -> None:
    """Summarize enrichment by kept-token-size bins with all points shown.

    This is more interpretable than a sparse scatter when n is small.
    """
    needed = {'theme', 'n_kept_tokens', 'refined_improve_enrichment_minus_log10_p'}
    if df.empty or not needed.issubset(df.columns):
        return
    g = df[['theme', 'n_kept_tokens', 'refined_improve_enrichment_minus_log10_p']].copy()
    g['n_kept_tokens'] = clean_numeric_series(g['n_kept_tokens'])
    g['refined_improve_enrichment_minus_log10_p'] = clean_numeric_series(g['refined_improve_enrichment_minus_log10_p'])
    g = g[np.isfinite(g['n_kept_tokens']) & np.isfinite(g['refined_improve_enrichment_minus_log10_p'])].copy()
    if g.empty:
        return
    ...
    order = ['1-2', '3-4', '5-6', '7+']
    g['size_bin'] = g['n_kept_tokens'].map(size_bin)
    present = [b for b in order if (g['size_bin'] == b).any()]
    if not present:
        return

    fig, ax = plt.subplots(figsize=(10, 5.5))
    rng = np.random.default_rng(0)
    x_positions = {b: i for i, b in enumerate(present)}
    medians = []
    counts = []
    for b in present:
        sub = g[g['size_bin'] == b].copy()
        x0 = x_positions[b]
        jitter = rng.uniform(-0.12, 0.12, size=len(sub))
        ax.scatter(np.full(len(sub), x0) + jitter, sub['refined_improve_enrichment_minus_log10_p'], alpha=0.85)
        med = float(np.nanmedian(sub['refined_improve_enrichment_minus_log10_p']))
        medians.append(med)
        counts.append(len(sub))
        ax.hlines(med, x0 - 0.22, x0 + 0.22, linewidth=2)
        # annotate strongest theme within each bin
        top = sub.sort_values('refined_improve_enrichment_minus_log10_p', ascending=False).iloc[0]
        ax.text(x0 + 0.16, float(top['refined_improve_enrichment_minus_log10_p']) + 0.02, str(top['theme']), fontsize=8)

    ax.set_xticks(range(len(present)))
    ax.set_xticklabels([f'{b}\n(n={n})' for b, n in zip(present, counts)])
    ax.set_ylabel(r'$-\log_{10}(p)$ for refined improve enrichment')
    ax.set_xlabel('kept-token count bin')
    ax.set_title('Size-matched view of refined improve enrichment')
    fig.tight_layout()
    fig.savefig(out_png, dpi=180)
    plt.close(fig)


def plot_size_bias_table_figure(df: pd.DataFrame, out_png: Path, topk: int = 10) -> None:
    """Render a compact table-like summary for the top refined themes.

    Useful when theme count is small and scatterplots are visually weak.
    """
    needed = {'theme', 'n_kept_tokens', 'n_drugs_supporting_refined_theme', 'refined_improve_enrichment_log2_odds_ratio', 'refined_improve_enrichment_minus_log10_p'}
    if df.empty or not needed.issubset(df.columns):
        return
    g = df.copy()
    for c in ['n_kept_tokens', 'n_drugs_supporting_refined_theme', 'refined_improve_enrichment_log2_odds_ratio', 'refined_improve_enrichment_minus_log10_p']:
        g[c] = clean_numeric_series(g[c])
    g = g[np.isfinite(g['refined_improve_enrichment_minus_log10_p'])].copy()
    if g.empty:
        return
    g = g.sort_values('refined_two_sided_minus_log10_p', ascending=False).head(topk).copy()
    show = g[['theme', 'n_kept_tokens', 'n_drugs_supporting_refined_theme', 'refined_improve_enrichment_log2_odds_ratio', 'refined_improve_enrichment_minus_log10_p']].copy()
    show = show.rename(columns={
        'theme': 'theme',
        'n_kept_tokens': 'kept_tokens',
        'n_drugs_supporting_refined_theme': 'theme_support',
        'refined_improve_enrichment_log2_odds_ratio': 'improve_vs_rest_log2OR',
        'refined_improve_enrichment_minus_log10_p': 'improve_vs_rest_-log10(p)',
    })
    show['kept_tokens'] = show['kept_tokens'].round(0).astype('Int64').astype(str)
    show['theme_support'] = show['theme_support'].round(0).astype('Int64').astype(str)
    show['improve_vs_rest_log2OR'] = show['improve_vs_rest_log2OR'].map(lambda x: f'{x:.3f}' if np.isfinite(x) else '')
    show['improve_vs_rest_-log10(p)'] = show['improve_vs_rest_-log10(p)'].map(lambda x: f'{x:.3f}' if np.isfinite(x) else '')

    fig_h = max(3.2, 0.42 * len(show) + 1.2)
    fig, ax = plt.subplots(figsize=(12, fig_h))
    ax.axis('off')
    tbl = ax.table(cellText=show.values, colLabels=show.columns, cellLoc='left', colLoc='left', loc='center')
    tbl.auto_set_font_size(False)
    tbl.set_fontsize(9)
    tbl.scale(1, 1.25)
    # highlight first row slightly
    for (row, col), cell in tbl.get_celld().items():
        if row == 0:
            cell.set_text_props(weight='bold')
        if row == 1:
            cell.set_text_props(weight='bold')
    ax.set_title('Top refined themes: improve-vs-worsen summary', pad=12)
    fig.tight_layout()
...
def main():
    ...
    theme_token_filtered_detail, theme_refined_token_set = build_theme_token_filtered_sets(
        raw_token_uni,
        raw_token_theme_catalog,
        raw_token_lodo,
        raw_token_thr_summary,
        min_support=2,
        min_abs_log2_or=0.25,
        min_lodo_consistency=0.80,
        min_threshold_consistency=0.80,
    )
    theme_token_filtered_detail.to_csv(outdir / "theme_token_filtered_detail.csv", index=False)
    theme_refined_token_set.to_csv(outdir / "theme_refined_token_set.csv", index=False)

    refined_theme_mat, refined_theme_catalog = build_refined_theme_presence_matrix(theme_token_filtered_detail, raw_token_mat, df['drug'])
    refined_theme_catalog.to_csv(outdir / "refined_theme_catalog.csv", index=False)
    refined_theme_mat.to_csv(outdir / "clean_refined_theme_presence_matrix.csv", index=False)
    df_refined_theme = df.merge(refined_theme_mat, on='drug', how='left', suffixes=('', '_refined_dup'))
    refined_dup_cols = [c for c in df_refined_theme.columns if c.endswith('_refined_dup')]
    if refined_dup_cols:
        df_refined_theme = df_refined_theme.drop(columns=refined_dup_cols)
    refined_theme_cols = [c for c in df_refined_theme.columns if c.startswith('refined_theme_')]
    for c in refined_theme_cols:
        df_refined_theme[c] = clean_numeric_series(df_refined_theme[c]).fillna(0).astype(int)

    refined_theme_uni = theme_univariate(df_refined_theme, refined_theme_cols, theme_prefix='refined_theme_')
    if not refined_theme_uni.empty:
        refined_theme_uni['theme'] = refined_theme_uni['theme'].astype(str)
    refined_theme_uni.to_csv(outdir / "refined_theme_univariate_improve_vs_worsen.csv", index=False)
    refined_theme_lodo = lodo_theme_stability(df_refined_theme, refined_theme_cols, theme_prefix='refined_theme_')
    if not refined_theme_lodo.empty:
        refined_theme_lodo['theme'] = refined_theme_lodo['theme'].astype(str)
    refined_theme_lodo.to_csv(outdir / "refined_theme_lodo_stability.csv", index=False)
    refined_theme_thr_detail, refined_theme_thr_summary = threshold_robustness_theme(df_refined_theme, refined_theme_cols, thresholds, theme_prefix='refined_theme_')
    refined_theme_thr_detail.to_csv(outdir / "refined_theme_threshold_robustness_detail.csv", index=False)
    refined_theme_thr_summary.to_csv(outdir / "refined_theme_threshold_robustness_summary.csv", index=False)
    refined_theme_consensus = build_theme_consensus(refined_theme_uni, refined_theme_lodo, refined_theme_thr_summary)
    refined_theme_consensus.to_csv(outdir / "refined_theme_feature_fairness_summary.csv", index=False)

    refined_enrich_improve = theme_enrichment(df_refined_theme, refined_theme_mat, 'improve', theme_prefix='refined_theme_')
    refined_enrich_worsen = theme_enrichment(df_refined_theme, refined_theme_mat, 'worsen', theme_prefix='refined_theme_')
    refined_enrich_iw = theme_enrichment(df_refined_theme[df_refined_theme['delta_group'].isin(['improve', 'worsen'])], refined_theme_mat, 'improve', 'worsen', theme_prefix='refined_theme_')
    refined_enrich_wi = theme_enrichment(df_refined_theme[df_refined_theme['delta_group'].isin(['improve', 'worsen'])], refined_theme_mat, 'worsen', 'improve', theme_prefix='refined_theme_')
    refined_enrich_improve.to_csv(outdir / "refined_category_enrichment_improve.csv", index=False)
    refined_enrich_worsen.to_csv(outdir / "refined_category_enrichment_worsen.csv", index=False)
    refined_enrich_iw.to_csv(outdir / "refined_category_enrichment_improve_vs_worsen.csv", index=False)
    refined_enrich_wi.to_csv(outdir / "refined_category_enrichment_worsen_vs_improve.csv", index=False)

    refined_theme_pvalue_comparison = compare_original_vs_refined_theme_pvalues(theme_uni, refined_theme_uni)
    refined_theme_pvalue_comparison.to_csv(outdir / "theme_pvalue_comparison_original_vs_refined.csv", index=False)

    refined_theme_size_bias = build_size_bias_table(refined_theme_catalog, refined_theme_uni, refined_enrich_improve, refined_enrich_worsen, theme_refined_token_set)
    refined_theme_size_bias.to_csv(outdir / "refined_theme_size_bias_table.csv", index=False)
    ...
    plot_barh(theme_refined_token_set, "theme", "weighted_mean_effect_kept_tokens", "Themes by refined token-set weighted effect", plots_dir / "top_theme_refined_weighted_mean_effect.png")
    plot_refined_theme_gain(theme_refined_token_set, plots_dir / "top_theme_refinement_gain.png")
    plot_barh(refined_theme_uni, "feature", "minus_log10_p", "Top refined themes: improve vs worsen", plots_dir / "top_refined_theme_univariate_minus_log10_p.png")
    plot_barh(refined_theme_consensus, "theme", "consensus_score", "Top refined themes by consensus", plots_dir / "top_refined_theme_consensus.png")
    plot_refined_theme_pvalue_gain(refined_theme_pvalue_comparison, plots_dir / "top_refined_theme_pvalue_gain.png")
    ...
    plot_barh(refined_theme_size_bias, "theme", "refined_improve_enrichment_log2_odds_ratio", "Refined themes: improve vs rest (theme-level log2 odds ratio)", plots_dir / "top_refined_enriched_categories_improve_vs_rest_log2_or_main.png")
    plot_barh(refined_enrich_worsen, "token", "minus_log10_p", "Refined themes enriched among worsened drugs", plots_dir / "top_refined_enriched_categories_worsen.png")
    plot_barh(refined_enrich_iw, "token", "minus_log10_p", "Refined themes enriched: improve vs worsen", plots_dir / "top_refined_enriched_categories_improve_vs_worsen.png")
    plot_barh(refined_enrich_wi, "token", "minus_log10_p", "Refined themes enriched: worsen vs improve", plots_dir / "top_refined_enriched_categories_worsen_vs_improve.png")

    plot_size_bias_ranked_panel(
        refined_theme_size_bias,
        plots_dir / 'size_bias_ranked_theme_panel.png',
        topk=12,
    )
    plot_size_matched_bin_summary(
        refined_theme_size_bias,
        plots_dir / 'size_bias_size_matched_bins.png',
    )
    plot_size_bias_table_figure(
        refined_theme_size_bias,
        plots_dir / 'size_bias_top_theme_table.png',
        topk=12,
    )
    ...
    if not theme_refined_token_set.empty:
        readme.append("")
        readme.append("Refined theme token sets (after within-theme denoising):")
        for _, r in theme_refined_token_set.head(10).iterrows():
            readme.append(f"- {r['theme']}: kept={int(r.get('n_tokens_kept', 0))}/{int(r.get('n_tokens_total', 0))}, weighted_all={safe_float(r.get('weighted_mean_effect_all_tokens')):.3f}, weighted_kept={safe_float(r.get('weighted_mean_effect_kept_tokens')):.3f}, gain={safe_float(r.get('refinement_gain_weighted_mean')):.3f}")
    if not refined_theme_pvalue_comparison.empty:
        readme.append("")
        readme.append("Original vs refined theme p-value comparison:")
        for _, r in refined_theme_pvalue_comparison.head(10).iterrows():
            readme.append(f"- {r['theme']}: original_p={safe_float(r.get('original_pvalue')):.3g}, refined_p={safe_float(r.get('refined_pvalue')):.3g}, minus_log10_gain={safe_float(r.get('minus_log10_p_gain')):.3f}")
    if not refined_theme_size_bias.empty:
        readme.append("")
        readme.append("Size-bias diagnostics for refined themes:")
        for _, r in refined_theme_size_bias.head(10).iterrows():
            readme.append(f"- {r['theme']}: n_kept_tokens={int(safe_float(r.get('n_kept_tokens', 0)) or 0)}, refined_support={int(safe_float(r.get('n_drugs_supporting_refined_theme', 0)) or 0)}, two_sided_minus_log10_p={safe_float(r.get('refined_two_sided_minus_log10_p')):.3f}, improve_enrichment_log2OR={safe_float(r.get('refined_improve_enrichment_log2_odds_ratio')):.3f}, improve_enrichment_minus_log10_p={safe_float(r.get('refined_improve_enrichment_minus_log10_p')):.3f}")
    ...
    for fn in [
        ...
        "theme_token_filtered_detail.csv",
        "theme_refined_token_set.csv",
        "clean_refined_theme_presence_matrix.csv",
        "refined_theme_catalog.csv",
        "refined_theme_univariate_improve_vs_worsen.csv",
        "refined_theme_lodo_stability.csv",
        "refined_theme_threshold_robustness_detail.csv",
        "refined_theme_threshold_robustness_summary.csv",
        "refined_theme_feature_fairness_summary.csv",
        "theme_pvalue_comparison_original_vs_refined.csv",
        "refined_theme_size_bias_table.csv",
        "refined_category_enrichment_improve.csv",
        "refined_category_enrichment_worsen.csv",
        "refined_category_enrichment_improve_vs_worsen.csv",
        "refined_category_enrichment_worsen_vs_improve.csv",
        "category_enrichment_improve.csv",
        "category_enrichment_worsen.csv",
        "category_enrichment_improve_vs_worsen.csv",
        "category_enrichment_worsen_vs_improve.csv",
